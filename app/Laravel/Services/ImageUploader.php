		...
		switch (Str::lower($storage)) {
			case 'file':
				...
				if($width >= 1024){
					...
					Image::make("{$path_directory}/{$filename}")->interlace()->widen(1024)->orientate()->save("{$resized_directory}/{$filename}",95);
					Image::make("{$path_directory}/{$filename}")->interlace()->widen(320)->orientate()->save("{$thumb_directory}/{$filename}",95);
					
				}else{
					Image::make("{$path_directory}/{$filename}")->interlace()->widen($width)->orientate()->save("{$resized_directory}/{$filename}",95);

					if($width >= 320){
						//if greater than or equalt to 320 width
						Image::make("{$path_directory}/{$filename}")->interlace()->widen(320)->orientate()->save("{$thumb_directory}/{$filename}",95);
					}else{
						Image::make("{$path_directory}/{$filename}")->interlace()->widen($width)->orientate()->save("{$thumb_directory}/{$filename}",95);
					}
			...
			case 'aws':
				...
				$new_image_filename = $filename;
				$file->move($path_directory, $filename); 
				Image::make(("{$path_directory}/{$filename}"))->interlace()->widen($resized_size)->orientate()->save(("{$resized_directory}/{$filename}"),100);
				Image::make(("{$path_directory}/{$filename}"))->interlace()->widen($thumbnail_size)->orientate()->save(("{$thumb_directory}/{$filename}"),100);
